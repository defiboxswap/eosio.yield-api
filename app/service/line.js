...
class LineService extends Service {
  ...
  async list(params) {
    ...
    // get history line
    const history_result = await ctx.service.cache.lru_computeIfAbsent(cache_key, async () => {
  ...
  async stat_list(params) {
    ...
    // get history line
    const history_result = await ctx.service.cache.lru_computeIfAbsent(cache_key, async () => {
      const sql = `select * from line_protocol_stat_${params.line_type} where line_id >= :from `;
      return await db.query(sql, { from });
    });

    // get latest line
    const sql = 'select * from protocol_stat where period > 0 ';
